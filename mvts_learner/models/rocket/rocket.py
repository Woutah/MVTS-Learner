# Sourced from https://github.com/ChangWeiTan/TS-Extrinsic-Regression/tree/aed493c4b089459f899da5f988569c11931d8fa7
# Some compatibility changes have been made - (enabling/disabling njtit etc.) - see the original code for the original version
...
from .ts_extrinsic__time_series_models import TimeSeriesRegressor
from .ts_extrinsic__tools import save_test_duration, save_train_duration
...
@njit(parallel=True, fastmath=True) #temporarily removed parallel + fastmath due to unexpected errors
...
class RocketRegressor(TimeSeriesRegressor):
	...
	def __init__(self,
		...
		self.regressor = RidgeCV(alphas=np.logspace(-3, 3, 10))
								#  normalize=True) #This was in the original code, but it seems to apply to a previous version of sklearn
	...
	def fit(self,
		...
		x_training_transform = apply_kernels(x_train, self.kernels) #Also converts (num_examples, num_timestep, num_channels) to (num_examples, num_kernels *2 (not exactly sure))
