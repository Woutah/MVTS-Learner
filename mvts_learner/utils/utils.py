#type: ignore
"""
Practically the same as original 
https://github.com/gzerveas/mvts_transformer/blob/3f2e378bc77d02e82a44671f20cf15bc7761671a/src/utils/utils.py
...
def save_model(path, epoch, model, optimizer=None):
	...
	# if isinstance(model, RocketClassiRegressor):
	# 	data["rocket_kernels"] = model._kernels #Also save the model kernels for future use

	torch.save(data, path)


def load_model(model, model_path, config, optimizer=None, resume=False, change_output=False,
			   lr=None, lr_step=None, lr_factor=None, ):
	#TODO: implement alternative model-loading?
	start_epoch = 0
...
def export_performance_metrics(filepath, metrics_table, header, book=None, sheet_name="metrics"):
	...
	book.save(filepath)
	log.info("Exported per epoch performance metrics in '{}'".format(filepath))
...
def register_record(filepath, timestamp, experiment_name, best_metrics, final_metrics=None, comment=''):
	...
	if len(best_metrics) ==0 :
		log.warning("No best metrics provided!, skipping register_record...")
		return
	...
	if not os.path.exists(filepath):  # Create a records file for the first time
		log.warning("Records file '{}' does not exist! Creating new file ...".format(filepath))
		...
		try:
			...
			log.error("Failed saving in: '{}'! Will save here instead: {}".format(filepath, alt_path))
			export_record(alt_path, row_values)
			filepath = alt_path

	log.info("Exported performance record to '{}'".format(filepath))


class Printer(object):
	"""
	Class for printing output by refreshing the same line in the console, e.g. for indicating progress of a
	process
	"""
...
def count_parameters(model, trainable=False):
	if not isinstance(model, torch.nn.Module): #Added 2023.01 to handle sklearn models
		return -1
	if trainable:
