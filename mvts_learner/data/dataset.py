...
ImputationDataset: Torch.Dataset subclass for imputation task
TransductionDataset: Torch.Dataset subclass for transduction task
ClassiregressionDataset: Torch.Dataset subclass for classification/regression task
...
class TransductionDataset(Dataset):
	"""
	A dataset for transduction tasks
	"""
...
def collate_superv(data, max_len=None): #type: ignore
...
class ClassiregressionDataset(Dataset):
	...
	def __init__(self, data, indices):
		...
		self.IDs = indices  # list of data IDs, but also mapping between integer index and ID
		self.feature_df = self.data.feature_df.loc[self.IDs]
		self.all_labels_df = self.data.all_labels_df.loc[self.IDs]
	...
	def __getitem__(self, ind):
		...
		X = self.feature_df.loc[self.IDs[ind]].values  # (seq_length, feat_dim) array
		y = self.all_labels_df.loc[self.IDs[ind]].values  # (num_labels,) array
...
def compensate_masking(X, mask):
	"""
	Compensate feature vectors after masking values, in a way that the matrix product W @ X would not be affected on
	average.
...
def collate_unsuperv(data, max_len=None, mask_compensation=False): #type: ignore
	"""Build mini-batch tensors from a list of (X, mask) tuples. Mask input. Create
	Args:
	...
	padding_masks = padding_mask(
		torch.tensor(lengths, dtype=torch.int16), max_len=max_len)  # (batch_size, padded_length) bool tensor, "1" = keep
...
def noise_mask(X,
		masking_ratio,
		lm : int =3, #pylint: disable=invalid-name
		mode='separate',
	...
	if distribution == 'geometric':  # stateful (Markov chain)
		if mode == 'separate':  # each variable (feature) is independent
			mask = np.ones(X.shape, dtype=bool)
			for m in range(X.shape[1]):  # feature dimension #pylint: disable=invalid-name
...
def geom_noise_mask_single(L, lm, masking_ratio): #pylint: disable=invalid-name
	"""
	Randomly create a boolean mask of length `L`, consisting of subsequences of average length lm, masking with 0s a 
	`masking_ratio` proportion of the sequence L. The length of masking subsequences and intervals follow a geometric 
	distribution.
	Args:
	...
	p_u = p_m * masking_ratio / (1 - masking_ratio)  # prob. of unmasked sequence stopping. param. of geometric dist.
	p = [p_m, p_u] #pylint: disable=invalid-name
