...
Datasets:
	- HDD_Data -  Hard Drive Disk failure dataset
	- WeldData - Welding dataset
	- TSRegressionArchive - Time Series Regression Archive (www.timeseriesregression.org)
	- TSClassificationArchive - Time Series Classification Archive (www.timeseriesclassification.com)


And help-classes:
	- Normalizer - Normalizes dataframe across ALL contained rows (time steps). Different from per-sample normalization.
"""
import glob
import logging
import os
...
from mvts_learner.data import utils
from mvts_learner.data.data_loader import load_data
from mvts_learner.data.data_loader.base_data import BaseData
...
class Normalizer(object):
	...
	def normalize(self, df):
		...
		if self.norm_type == "standardization":
			if self.mean is None or self.std is None: #Recalculate mean and std if not provided
				self.mean = df.mean()
				self.std = df.std()
...
class HDD_Data(BaseData): #pylint: disable=invalid-name #HDDData not very clear
	"""
	...
	Attributes:
		all_df: (num_samples * seq_len, num_columns) dataframe indexed by integer indices, with multiple rows
			corresponding to the same index (sample).
	...
	def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
		...
		self.failed_IDs = self.all_df[ #pylint: disable=invalid-name
			self.all_df.failure == 1].index.unique()  # IDs corresponding to assets which failed
		self.normal_IDs = sorted( #pylint: disable=invalid-name
			list(set(self.all_IDs) - set(self.failed_IDs)))  # IDs corresponding to assets without failure

		self.feature_df = self.all_df #TODO: check if this is correct
	...
	def load_all(self, dir_path):
		...
		if self.n_proc > 1:
			...
			logger.info(f"Loading {len(input_paths)} datasets files using {_n_proc} parallel processes ...")
	...
	def process_columns(df):
		"""Process columns to correct datatypes"""
...
class WeldData(BaseData):
	...
	def load_all(self, root_dir, file_list=None, pattern=None):
		...
		if len(data_paths) == 0:
			raise FileNotFoundError(f"No files found using: {os.path.join(root_dir, '*')}")
		...
		if len(input_paths) == 0:
			raise FileNotFoundError(f"No .csv files found using pattern: '{pattern}'")
	...
	@staticmethod
	def load_single(filepath):
		"""Load single weld-data file"""
		df = WeldData.read_data(filepath)
		df = WeldData.select_columns(df)
		num_nan = df.isna().sum().sum()
		if num_nan > 0:
			logger.warning(f"{num_nan} nan values in {filepath} will be replaced by 0")
			df = df.fillna(0)
	...
	@staticmethod
	def select_columns(df):
		"""Retrieves only the target columns (features) from the dataframe"""
...
class TSRegressionArchive(BaseData):
	...
	Attributes:
		all_df: (num_samples * seq_len, num_columns) dataframe indexed by integer indices, with multiple rows
			corresponding to the same index (sample). Each row is a time step; Each column contains either metadata
			(e.g. timestamp) or a feature.
		feature_df: (num_samples * seq_len, feat_dim) dataframe; contains the subset of columns of `all_df` which
			correspond to selected features
		feature_names: names of columns contained in `feature_df` (same as feature_df.columns)
		all_IDs: (num_samples,) series of IDs contained in `all_df`/`feature_df` (same as all_df.index.unique() )
		all_labels_df: (num_samples, num_labels) pd.DataFrame of label(s) for each sample
		max_seq_len: maximum sequence (time series) length. If None, script argument `max_seq_len` will be used.
			(Moreover, script argument overrides this attribute)
	"""
	...
	def __init__(self,
		...
		assert config is not None, "TSRegressionArchive requires a config object"
		self.max_seq_len = None
		self.config : TsRegressionDatasetOptions = config #Config should contain a TSRegressionDatasetOptions object
		self.class_names = None
		self.all_df, self.all_labels_df = self.load_all(root_dir, file_list=file_list, pattern=pattern)
	...
	def load_all(self, root_dir, file_list=None, pattern=None):
		...
		Returns:
			all_df: a single (possibly concatenated) dataframe with all data corresponding to specified files
			all_labels_df: dataframe containing label(s) for each sample
		"""
		...
		if self.config.load_path_is_pkl_dict:
			input_paths = [p for p in selected_paths if os.path.isfile(p) and p.endswith('.dill')] #If loading from dill
		else:
			input_paths = [p for p in selected_paths if os.path.isfile(p) and p.endswith('.ts')]
		if len(input_paths) == 0:
			raise FileNotFoundError(f"No .ts files found using pattern: '{pattern}'")

		all_df, all_labels_df = self.load_single(input_paths[0])  # a single file contains dataset

		return all_df, all_labels_df
	...
	def load_single(self, filepath):
		...
		if self.config.load_path_is_pkl_dict:
			#Load df and labels from dill instead of a raw tsregression-archive file
			with open(filepath, "rb") as file:
				loaded = pickle.load(file)
				df = loaded["df"]
				all_labels_df = loaded["labels"]
			...
			all_labels_df = pd.DataFrame(labels, dtype=np.float32)
		elif self.config['task'] == 'classification':
			loaded = load_data.load_from_tsfile_to_dataframe(
				filepath, return_separate_X_and_y=True, replace_missing_vals_with='NaN')
			df, labels = loaded
			labels = pd.Series(labels, dtype="category")
			self.class_names = labels.cat.categories
			all_labels_df = pd.DataFrame(labels.cat.codes, dtype=np.int8)  #int32 errors when using nn.CrossEntropyLoss
		else:  # e.g. imputation
			try:
		...
		if self.config.save_loaded_to_pkl and not self.config.load_path_is_pkl_dict:
			#Split filepath such that extension is removed
			filepath_split = filepath.rsplit(".")[0]
			if os.path.isfile(filepath_split + ".pkl") and not self.config.overwrite_save_to_pkl:
				logger.warning(f"File {filepath_split}.pkl already exists. Not saving to pkl.")
			else:
				#Save df and labels to dill
				with open(filepath_split + ".dill", "wb") as file:
					pickle.dump(
						{
							"df": df,
							"labels": all_labels_df,
		...
		sub_factor = self.config['subsample_factor']
		if sub_factor:
			df = df.applymap(lambda x: subsample(x, limit=0, factor=sub_factor))

		lengths = df.applymap(len).values
		vert_diffs = np.abs(lengths - np.expand_dims(lengths[0, :], 0))
		if np.sum(vert_diffs) > 0:  # if any column (dimension) has varying length across samples
			self.max_seq_len = int(np.max(lengths[:, 0]))
			logger.warning(f"Not all samples have same length: maximum length set to {self.max_seq_len}")
		else:
...
class SemicondTraceData(BaseData):
	...
	Attributes:
		...
		max_seq_len: maximum sequence (time series) length. If None, script argument `max_seq_len` will be used.
			(Moreover, script argument overrides this attribute)
	"""
	# TODO: currently all *numeric* features which are not *dataset-wise* constant are kept.
	# Sample-wise constants are included
	...
	def __init__(self, root_dir, file_list=None, pattern=None, n_proc=8, limit_size=None, config=None):
		assert config is not None, "SemicondTraceData requires a config object to read in settings."
		self.set_num_processes(n_proc=n_proc)

		## Get labels
		wafer_measurements_path = os.path.join(root_dir, "waferdata/")
		logger.info("Getting wafer measurements ...")
		# Dataframe which holds all measurements: mean thickness (and deposition rate for the subset "type 1"),
		# roughness (std of thickness)
		...
		# This dataframe holds for all wafers all metadata per wafer (including measurements, when they exist,
		# and corresponding trace file)
		...
		files_20A = metadata_df.loc[ #pylint: disable=invalid-name
			metadata_df['ChamberRecipeID'] == 'QUALCH2CO20', 'TraceDataFile']
		# IDs_20A = files_20A.index
		files_20A = list(map(self.convert_tracefilename, files_20A)) #pylint: disable=invalid-name
		...
		if config['task'] == 'regression':
			...
			self.all_labels_df = pd.DataFrame(metadata_df.loc[self.all_IDs, labels_col], dtype=np.float32)
			self.all_labels_df = self.all_labels_df[~self.all_labels_df[labels_col].isna()]
			self.all_IDs = self.all_labels_df.index
			self.all_df = self.all_df.loc[self.all_IDs]
	...
	def make_pjid(self, toolID, pjID): #pylint: disable=invalid-name
	...
	def get_measurements(self, wafer_measurements_path):
		...
		deprate_df = pd.merge(deprate_df1, deprate_df2, how='outer', left_on=['Proc_cj_id', 'Wafer_id'],
							  ...
							  validate=None) #type: ignore #None=default, bug with pylint?
		...
		# Dataframe which holds all measurements:
		# 	mean thickness (and deposition rate for the subset "type 1"),
		# 	roughness (std of thickness)
	...
	def get_metadata(self, catalog_path, measurements_df):
		"""Return Semiconductor trace data metadata"""
	...
	@staticmethod
	def load_single(filepath):
		"""Load single semicond-trance file"""
		...
		feat_col = [col for col in df.columns if col in SemicondTraceData.features]# some columns are missing in tracefiles
...
class PMUData(BaseData):
	...
	def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config =None):
		assert config is not None, "PMUData requires a config object to read in settings."
		self.set_num_processes(n_proc=n_proc)
...
#Utility type containing all data-classes:
ALL_DATALOADER_CLASSES = typing.Union[WeldData, HDD_Data, TSRegressionArchive, SemicondTraceData, PMUData, PandasData] #pylint: disable=invalid-name

data_factory : typing.Dict[str, type[ALL_DATALOADER_CLASSES]]= {
	'weld': WeldData,
	'hdd': HDD_Data,
